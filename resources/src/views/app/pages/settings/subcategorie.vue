<template>
  <div class="main-content">
    <breadcumb :page="$t('SubCategories')" :folder="$t('Settings')"/>
      ...
      >
        ...
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            <a @click="Edit_subcategory(props.row)" title="Edit" v-b-tooltip.hover>
              <i class="i-Edit text-25 text-success"></i>
            </a>
            <a title="Delete" v-b-tooltip.hover @click="Remove_SubCategory(props.row.id)">
    ...
    <validation-observer ref="Create_Category">
      <b-modal hide-footer size="md" id="New_Category" :title="editmode?$t('Edit'):$t('Add')">
        <b-form @submit.prevent="Submit_SubCategory">
          <b-row>
            <!-- Code category -->
            <b-col md="12">
              <validation-provider
              ...
              >
                <b-form-group :label="$t('Codesubcategorie')">
                  <b-form-input
                    :placeholder="$t('Enter_Code_subcategory')"
                    :state="getValidationState(validationContext)"
                    aria-describedby="Code-feedback"
                    label="Code"
                    v-model="sub_category.code"
                  ></b-form-input>
            ...
            <b-col md="12">
              <validation-provider
                ...
                v-slot="validationContext"
              >
                <b-form-group :label="$t('Namesubcategorie')">
                  <b-form-input
                    :placeholder="$t('Enter_Name_subcategory')"
                    :state="getValidationState(validationContext)"
...
export default {
  ...
  computed: {
    columns() {
      return [
        {
          label: this.$t("Codesubcategorie"),
          field: "code",
          tdClass: "text-left",
          thClass: "text-left"
        },
        {
          label: this.$t("Namesubcategorie"),
          field: "name",
          tdClass: "text-left",
          thClass: "text-left"
        },
  ...
  methods: {
    ...
    Submit_SubCategory() {
      this.$refs.Create_Category.validate().then(success => {
        if (!success) {
          ...
          if (!this.editmode) {
            this.Create_SubCategory();
          } else {
            this.Update_SubCategory();
          }
        }
      });
    },
    ...
    Update_SubCategory() {
      this.SubmitProcessing = true;
      axios
        .put("subcategories/" +  this.sub_category.id, {
          name: this.sub_category.name,
          code: this.sub_category.code,
          category_id: this.sub_category.category_id
        })
