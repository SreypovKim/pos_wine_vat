                  ...
                  <validation-provider name="is_stock" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Is Stock')">
                      <v-select
                        ...
                        :options="stocks.map(stocks => ({label: stocks.name, value: stocks.is_stock}))"
                      />
                      <b-form-invalid-feedback>{{ errors[0] }}</b-form-invalid-feedback>
                    </b-form-group>
                  </validation-provider>
                                ...
                                <tr  v-for='combine, index in combines'>  
                                  <td>
                                    <v-select
                                      ...
                                      @input="changedValue(combines[index].product_combine_id,index)" 
                                    />
...
export default {
  ...
  data() {
    return {
      ...
      subcategories:"",
      units: [],
      stocks: [{'name': 'Stock', 'is_stock' : 1},{'name': 'No Stock', 'is_stock': 0}],
      units_sub: [],
      brands: [],
      roles: {},
      variants: [],
    ...
    combineDeleteRow: function (index) {
     ...
     ChooseCategory(id) {
       axios
        ...
        .then(response => {
          // if(response.data.sub_categories[0] == " " || response.data.sub_categories[0] == null){
    ...
    GetElements() {
      ...
      axios
        ...
        .then(response => {
          ...
          this.combines = response.data.product.ProductCombine;
          if(this.product.category_id){
            var id = this.product.category_id;
             axios
             .get(
                "subcategories/"+id 
              )
              .then(response => {
                  this.subcategories = response.data.sub_categories;
                  this.product.subcategory_id =   this.product.subcategory_id ;
    ...
    Update_Product() {
      ...
      if (self.combines.length) {
        for (var i = 0; i < self.combines.length; i++) {
          Object.entries(self.combines[i]).forEach(([key, value]) => {
            self.data.append("combines[" + i + "][" + key + "]", value);
          });
        }
      }
