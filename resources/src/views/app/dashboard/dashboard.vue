      ...
      <b-col lg="3" md="6" sm="12">
        <b-card class="card-icon-bg card-icon-bg-primary o-hidden mb-30 text-center">
          ...
          <div class="content">
            ...
            <p
              class="text-primary text-24 line-height-1 mb-2"> {{currentUser.currency}}{{formatNumber((report_today.revenue?report_today.revenue:0),2)}}</p>
          </div>
        </b-card>
      </b-col>
      ...
      <div class="col-md-12">
        <div class="card mb-30">
          <div class="card-body p-0">
            <h5 class="card-title border-bottom p-3 mb-2">{{$t('SalesAlert')}}</h5>

            <vue-good-table
                v-if="!isLoading"
                mode="remote"
                :columns="columns_sale_alert"
                :totalRows="totalRows"
                :rows="sale_alerts"
              ...
              <!-- <template slot="table-row" slot-scope="props">
                <div v-if="props.column.field == 'payment_statut'">
                  <span
                    v-if="props.row.payment_status == 'paid'"
                    class="badge badge-outline-success"
                  >{{$t('Paid')}}</span>
    ...
    report_with_echart() {
      axios
        ...
        .then(response => {
          ...
          this.sale_alerts = response.data.sale_alert.original.sale;
          this.totalRows = response.data.sale_alert.original.count;    
          var dark_heading = "#c2c6dc";
          // console.log(response.data.report_dashboard.original.test);
