  ...
  <div class="main-header">
    ...
    <div class="header-part-right">
      ...
      <div class="dropdown">
        ...
        >
          <template slot="button-content" >
            <span class="badge badge-primary" v-if="notifs_alert || due_date_count > 0">1</span>
            <span class="badge badge-primary" v-if="notifs_alert && due_date_count > 0">2</span>
          ...
          >
            <div class="dropdown-item d-flex" v-if="notifs_alert > 0">
             ...
             <!-- Due Date -->
             <div class="dropdown-item d-flex" v-if="due_date_count > 0">
              <div class="notification-icon">
                <i class="i-Bell text-primary mr-1"></i>
              </div>
              <div class="notification-details flex-grow-1"
             >
               <router-link  tag="a" to="/app/reports/due_date_alerts" >
                <p class="text-small text-muted m-0">
                  {{due_date_count}} {{$t('PaymentTermAlerts')}}
                  </p>
               </router-link>
              </div>
            </div>
...
export default {
  ...
  methods: {
   ...
   mounted(){
   axios
       .get('/report/sale_due_date')
       .then(response => (
        this.due_date_count = response.data.count
       ))
       }
};
</script>
