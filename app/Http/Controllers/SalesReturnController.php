...
class SalesReturnController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        foreach ($SaleReturn as $Sale_Return) {
            ...
            $item['date'] =  Carbon::parse($Sale_Return->date)->format('d-m-Y');
        ...
        \DB::transaction(function () use ($request, $id) {
                            ...
                            if ($unit && $product_warehouse) {
                                ...
                                $product_warehouse->save();
                                 if($product['is_combine']){
                                    $combine_products = Combine::where('product_id',$value['product_id'])->get();
                                    foreach($combine_products as  $combine_product){
                                        $product_combine_warehouses = product_warehouse::where('warehouse_id', $current_SaleReturn->warehouse_id)
                                        ->where('product_id',$combine_product->product_combine_id)
                                        ->get();
                                        $new_qty = $combine_product->qty * $value['quantity'];
                                        foreach($product_combine_warehouses as  $product_combine_warehouse){
                                            $product_whs = product_warehouse::where('warehouse_id', $current_SaleReturn->warehouse_id)
                                            ->where('product_id',$product_combine_warehouse->product_id)
                                            ->first();
                                                if ($unit->operator == '/') {
                                                    $product_combine_warehouse->qte -=  $new_qty / $unit->operator_value;
            ...
            foreach ($new_return_details as $key => $product_detail) {
                ...
                if($product_detail['no_unit'] !== 0){
                             ...
                             $product_warehouses = product_warehouse::where('deleted_at', '=', null)
                                ...
                                foreach( $product_warehouses as  $product_warehouse ){
                                    ...
                                    if ($product_warehouse_id) {
                                        ...
                                        if($product['is_combine']){
                                        $combine_products = Combine::where('product_id',$product_detail['product_id'])->get();
                                        foreach($combine_products as  $combine_product){
                                            $product_combine_warehouses = product_warehouse::where('warehouse_id', $request->warehouse_id)
                                            ->where('product_id',$combine_product->product_combine_id)
                                            ->get();
                                            $new_qty = $combine_product->qty * $product_detail['quantity'];
                                            foreach($product_combine_warehouses as  $product_combine_warehouse){
                    ...
                    $orderDetails['price'] = $product_detail['Net_price'];
    ...
    public function edit(Request $request, $id)
    {
        ...
        foreach ($SaleReturn['details'] as $detail) {
            ...
            if ($detail->product_variant_id) {
                ...
                // $data['product_variant_qty'] =   $detail->quantity / $detail->$qty;
