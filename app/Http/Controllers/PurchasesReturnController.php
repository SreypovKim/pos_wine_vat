...
class PurchasesReturnController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        foreach ($PurchaseReturn as $Purchase_Return) {
            ...
            $item['date'] =  Carbon::parse($Purchase_Return->date)->format('d-m-Y');
        ...
        \DB::transaction(function () use ($request, $id) {
                    ...
                    if ($current_PurchaseReturn->statut == "completed") {
                        if ($value['product_variant_id'] !== null) {
                            ...
                            $product_warehouses = product_warehouse::where('deleted_at', '=', null)
                                ...
                                ->where('product_id', $value['product_id'])->get();
                                // ->where('product_variant_id', $value['product_variant_id'])->get();
                            //     ->first();
            ...
            foreach ($New_Return_Details as $key => $product_detail) {
                ...
                if($product_detail['no_unit'] !== 0){
                    ...
                    if ($request->statut == "completed") {
                        if ($product_detail['product_variant_id'] !== null) {
                            $product_warehouses = product_warehouse::where('deleted_at', '=', null)
                                ...
                                // ->first();
                                    ->get();
                                    $product_variant = ProductVariant::find($product_detail['product_variant_id']);
                                    $qty =   $product_variant['qty'] *  $product_detail['quantity'];
                                    foreach( $product_warehouses as  $product_warehouse ){
                                        $product = product_warehouse::find($product_warehouse->id);
                    ...
                    $orderDetails['cost'] = $product_detail['Net_cost'];
    ...
    public function edit(Request $request, $id)
    {
        ...
        foreach ($Purchase_Return['details'] as $detail) {
            ...
            if ($detail->product_variant_id) {
                ...
                $data['product_variant_name'] = $productsVariants ? $productsVariants->name : $unit->ShortName;
                $data['product_qty'] = $productsVariants ? $item_product->qte / $productsVariants->qty : $item_product->qte;
                ...
                $item_product = product_warehouse::where('product_id', $detail->product_id)
                    ...
                    $data['product_variant_name'] = $unit->ShortName;
                    $data['product_qty'] = $item_product->qte;
