...
class ReportController extends BaseController
{
    ...
    public function report_with_echart()
    {
        ...
        $sale_alert = $this->count_due_date_alert();
    ...
    public function report_dashboard()
    {
        ...
        $product_sale_data = Sale::join('sale_details' , 'sales.id', '=', 'sale_details.sale_id')
            ...
            ->whereBetween('sales.date',[$date." 00:00:00",$date." 23:59:59"])
        ...
        $date = $data['revenue'] + $data['purchases_return'] - $product_cost ;
     
        return response()->json([
            // 'test'=>$product_purchase_data,
            'date'=> $date,
            'products' => $products,
    ...
    public function Sales_Client(request $request)
    {
     ...
     public function Sales_Client_Service(request $request)
    {
        ...
        $sales = Sale::where('deleted_at', '=', null)->with(['client','serviceDetails'])
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
                    return $query->where('user_id', '=', Auth::user()->id);
                }
        ...
        $data = [];
        foreach ($sales as $sale) {
            if(!empty($sale['serviceDetails'])){
                   foreach ($sale['serviceDetails'] as $val) {
                        $item['date'] = Carbon::parse($sale->date)->format('d-m-Y');
                        $item['name'] = $val['service']->name;
    ...
    public function Report_Sales(request $request)
    {
        ...
        $helpers = new helpers();
        // Filter fields With Params to retrieve
        $param = array(0 => 'like', 1 => 'like', 2 => '=', 3 => 'like' , 4 => '=',5 => '=');
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where('sales.Ref', 'LIKE', "%{$request->search}%")
                        ...
                        ->orWhereBetween('sale.date',[$start_date." 00:00:00",$end_date." 23:59:59"])
                        ->orWhere('sales.date', $request->search)
        ...
        return response()->json(['totalRows' => $totalRows, 'sales' => $data, 'customers' => $customers,'Total'=>  $total, 'val' => $val]);
    }
    ...
    public function Quotations_Warehouse(request $request)
    {
        ...
        foreach ($Quotations as $Quotation) {
            $item['date'] = Carbon::parse( $Quotation->date)->format('d-m-Y');
    ...
    public function Expenses_Warehouse(request $request)
    {
        ...
        foreach ($Expenses as $Expense) {
            ...
            $item['date'] = Carbon::parse($Expense->date)->format('d-m-Y');
    ...
    public function count_due_date_alert(){
        $today = Carbon::now()->format('Y-m-d');
        $sale = Sale::where('deleted_at', '=', null)->where('exp_date','<=',$today)->where('payment_statut','!=','paid');
        $sales =  $sale->get();
        $count = $sale->count();

        $data = [];
        foreach ($sales  as $sale ) {
            $item['customer_name'] = $sale['client']->name;
            $item['phone'] = $sale['client']->phone;
            $item['total_price'] = $sale->GrandTotal;
            $item['payment_statut'] = $sale->payment_statut;
            $item['exp_date'] = $sale->exp_date;
            $item['piad'] = $sale->paid_amount;
            $item['due'] =  $item['total_price'] - $item['piad'];
