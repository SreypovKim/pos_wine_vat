...
class PurchasesController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        foreach ($Purchases as $Purchase) {
            ...
            $item['id'] = $Purchase->id;
            $item['date'] = Carbon::parse($Purchase->date)->format('d-m-Y');
    ...
    public function store(Request $request)
    {
        ...
        \DB::transaction(function () use ($request) {
            ...
            foreach ($data as $key => $value) {
                ...
                $find_product = Product::Find($value['product_id']);
                // $find_product->update(['cost' => $value['Net_cost']]);

                $product_variant = ProductVariant::find($value['product_variant_id']);
                if($product_variant){
                    $purchase_cost =  $value['Net_cost'] / $product_variant->qty;
                }
                else{
                    $purchase_cost = $value['Net_cost'];
                }
                $find_product->update(['cost' => $purchase_cost]);
                ...
                if ($order->statut == "received") {
                    if ($value['product_variant_id'] !== null) {
                        ...
                        $product_variant = ProductVariant::find($value['product_variant_id']);
                        $qty =  $value['quantity'] * $product_variant->qty;
                        foreach( $product_warehouses as $product_warehouse){
                            $product = product_warehouse::find($product_warehouse->id);
                            if ($unit && $product ) {
                                if ($unit->operator == '/') {
                                
                                    $product->qte +=  $qty / $unit->operator_value;
        ...
        \DB::transaction(function () use ($request, $id) {
                        ...
                        if ($value['product_variant_id'] !== null) {
                            $product_warehouses = product_warehouse::where('deleted_at', '=', null)
                                ...
                                foreach($product_warehouses as $product_warehouse){
                                    ...
                                    if ($unit && $product_warehouse_id) {
                                        if ($unit->operator == '/') {
                                            $product_warehouse_id->qte -=  $qty  / $unit->operator_value;
                                        } else {
                                            $product_warehouse_id->qte -= $qty  * $unit->operator_value;
                                        }
            ...
            foreach ($new_purchase_details as $key => $prod_detail) {
                ...
                if($prod_detail['no_unit'] !== 0){
                    ...
                    if ($request['statut'] == "received") {
                        ...
                        if ($prod_detail['product_variant_id'] !== null) {
                            $product_warehouses = product_warehouse::where('deleted_at', '=', null)
                             ...
                             foreach( $product_warehouses as  $product_warehouse ){
                                $product = product_warehouse::find($product_warehouse->id);
                                if ($unit_prod && $product) {
                                    if ($unit_prod->operator == '/') {
                                        $product->qte += $qty / $unit_prod->operator_value;
                                        // $product->qte =$prod_detail['quantity'];
                                    } else {
                                        // $product->qte += $prod_detail['quantity'] * $unit_prod->operator_value;
                                        $product->qte += $qty * $unit_prod->operator_value;
                                    }
                    ...
                    $orderDetails['cost'] =$prod_detail['Net_cost'];
                    ...
                    // $find_product = Product::Find( $prod_detail['product_id']);
                    $find_product = Product::Find($prod_detail['product_id']);
                    // $find_product->update(['cost' => $prod_detail['Net_cost']]);
                    $product_variant = ProductVariant::find($prod_detail['product_variant_id']);
                    if($product_variant){
                        $purchase_cost =  $prod_detail['Net_cost'] / $product_variant->qty;
                    }
                    else{
                        $purchase_cost = $value['Net_cost'];
                    }
                    ...
                    //$find_product->update(['cost' => $purchase_cost > 0 ? $purchase_cost : $prod_detail['Net_cost']]);
                    // $find_product->update(['cost' => $prod_detail['Net_cost']]);
                }
            }
    ...
    public function show(Request $request, $id)
    {
        ...
        foreach ($purchase['details'] as $detail) {
            ...
            if ($detail->product_variant_id) {
                ...
                $data['productsVariantQty'] = $productsVariants->qty / $detail->quantity ;
                $data['productsVariantName'] = $productsVariants->name;
           
           
            } else {
                $data['code'] = $detail['product']['code'];
                $data['productsVariantName'] = $unit->ShortName;
            }
    ...
    public function edit(Request $request, $id)
    {
        ...
        foreach ($Purchase_data['details'] as $detail) {
            ...
            if ($detail->product_variant_id) {
                ...
                $data['product_variant_name'] =  $productsVariants->name;               
